...
function MyReadingLogsPage() {
  ...
  const readingBooks = shelfBooks.filter(
    (book) => book.totalBookPages <= 0 || book.approvedPages < book.totalBookPages,
  );
  const completedBooks = shelfBooks.filter(
    (book) => book.totalBookPages > 0 && book.approvedPages >= book.totalBookPages,
  );
  const visibleLogs = showAllLogs ? logs : logs.slice(0, PREVIEW_LOG_COUNT);

  return (
    <main className="log-page my-library-page">
      <section className="my-library-hero">
        <div>
          <p className="log-label">My Library</p>
          <h1>나의 서재</h1>
          <p>읽고 있는 책과 완독한 책을 한눈에 확인해 보세요.</p>
        </div>
      ...
      {isLoading ? (
        <div className="my-library-state">나의 서재를 불러오고 있습니다.</div>
      ) : errorMessage ? (
        <div className="my-library-state reading-log-error">{errorMessage}</div>
      ) : (
        <>
          ...
          <div className="my-library-shelves">
            <BookShelf
              title="읽고 있는 책"
              description="최근에 펼쳐 본 책부터 보여드려요."
              books={readingBooks}
              emptyMessage="현재 읽고 있는 책이 없습니다."
            />
            <BookShelf
              title="완독한 책"
              description="인정된 독서 페이지를 기준으로 완독한 책이에요."
              books={completedBooks}
              emptyMessage="아직 완독한 책이 없습니다."
            />
          </div>

          <section className="my-library-recent">
            <div className="my-library-recent-head">
              <div>
                <h2>최근 독서일지</h2>
                <p>제출 상태와 상세 기록을 확인할 수 있습니다.</p>
              </div>
              {logs.length > PREVIEW_LOG_COUNT && (
                <button
                  type="button"
                  className="my-library-more"
            ...
            {visibleLogs.length === 0 ? (
              <div className="my-library-empty">아직 제출한 독서일지가 없습니다.</div>
            ) : (
              <div className="my-library-log-list">
                {visibleLogs.map((log) => {
                  ...
                  return (
                    <Link
                      ...
                      to={`/logs/${log.readingLogId}`}
                      className="my-library-log-row"
                    >
                      ...
                      <span className="my-library-log-arrow" aria-hidden="true">
                        →
                      </span>
                    </Link>
                  );
