          ...
          {useSimpleLayout ? (
            <div className="ranking-race-tracks ranking-race-tracks--simple">
              ...
              <div className="ranking-race-progress-meta">
                <span>{formatDistance(ranking.distance)} · {ranking.approvedPages.toLocaleString()}쪽</span>
                <small><i aria-hidden="true" /> 승인 {ranking.approvedPages.toLocaleString()}쪽</small>
              </div>
            </div>
          ) : (
            <div className="ranking-race-tracks">
              <div className="ranking-race-track is-distance"><span className="ranking-start-mark" aria-hidden="true">START</span><span className="ranking-race-fill" style={{ width: `${progress}%` }} /><span className="ranking-runner" style={{ left: `${runnerPosition}%` }} aria-hidden="true"><svg viewBox="0 0 24 24"><circle cx="15" cy="4" r="2"/><path d="m13 7-3 5 4 2 2 6M11 10l-4-2-3 3M10 12l-3 7M14 9l5 3"/></svg></span><i className="ranking-finish-mark" aria-hidden="true">FINISH</i><b>{formatDistance(ranking.distance)}</b></div>
